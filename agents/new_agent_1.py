...
class NewAgent1(Agent):
    """Robust agent with direct, bank, and safety candidates."""

    def __init__(self):
        super().__init__()
        self._default_ball_radius = 0.028575
        self._sim_noise = {"V0": 0.1, "phi": 0.15, "theta": 0.1, "a": 0.004, "b": 0.004}
        self._max_base_candidates = 30
        self._rollouts = 6
        self._max_cut_angle = 65.0
        self._bank_penalty = 1.7
    ...
    def decision(self, balls=None, my_targets=None, table=None):
        ...
        candidates.extend(self._build_safety_candidates(cue_pos, remaining_targets, balls, table, ball_r))
        candidates.extend(self._random_candidates())

        if not candidates:
            return self._random_action()

        candidates.sort(key=lambda x: x[0], reverse=True)
        base_top = candidates[: self._max_base_candidates]
        expanded = self._expand_spin_variants(base_top)
        expanded.sort(key=lambda x: x[0], reverse=True)
        top = expanded[: self._max_base_candidates]

        best = self._evaluate_candidates(balls, table, remaining_targets, top)
        return best if best is not None else self._random_action()
    ...
    def _scratch_risk_penalty(self, cue_pos, aim_dir, table, ball_r, v0):
        ...
        for p in table.pockets.values():
            ...
            if lateral < 2.6 * ball_r and along < 0.9:
                if remaining > 3:
                    penalty += 2.2 * speed_scale
                else:
                    penalty += 1.5 * speed_scale
        return penalty
    ...
    def _build_direct_candidates(
        ...
        if self._is_path_blocked(cue_pos, ghost_pos, balls, exclude_ids={"cue", target_id}, clearance=2.2 * ball_r):
            return []
        ...
        for delta in angle_jitter:
            for scale in speed_scales:
                ...
                v0 = float(np.clip(base_v0 * scale, 0.8, 5.8))
                action = {
                    "V0": v0,
                    "phi": adj_phi,
                    "theta": 0.0,
    ...
    def _bank_point(self, obj_pos, pocket_pos, bounds, side):
        ...
        if side in {"left", "right"}:
            ...
            if y < min_y or y > max_y:
                return None
            return np.array([line_x, y], dtype=float)
        ...
        if x < min_x or x > max_x:
            return None
        return np.array([x, line_y], dtype=float)
    ...
    def _build_bank_candidates(
        ...
        for side in ("left", "right", "bottom", "top"):
            bank_point = self._bank_point(obj_pos, pocket_pos, bounds, side)
            if bank_point is None:
                continue
            reflected = self._reflect_point(pocket_pos, "x" if side in {"left", "right"} else "y", bank_point[0] if side in {"left", "right"} else bank_point[1])
            ...
            cut_cos = float(np.clip(np.dot(cue_to_obj_dir, to_pocket_dir), -1.0, 1.0))
            cut_angle = float(math.degrees(math.acos(cut_cos)))
            if cut_angle > (self._max_cut_angle - 8.0):
                continue
    ...
    def _build_safety_candidates(self, cue_pos, remaining_targets, balls, table, ball_r):
        out = []
        for tid in remaining_targets:
            ...
            base_v0 = float(np.clip(0.9 + 0.5 * dist, 0.8, 2.8))
            score = -1.2 * dist - 0.2 * self._scratch_risk_penalty(cue_pos, dir2, table, ball_r, base_v0)
            for scale in (0.8, 1.0, 1.2):
                v0 = float(np.clip(base_v0 * scale, 0.8, 3.0))
                action = {
                    "V0": v0,
                    "phi": phi,
                    "theta": 0.0,
    ...
    def _random_candidates(self):
        ...
        for _ in range(6):
            ...
            action["_meta"] = {"type": "random"}
            out.append((-3.0, action))
        return out
    ...
    def _expand_spin_variants(self, candidates):
        ...
        for score, action in candidates:
            for theta, a, b in spin_variants:
                ...
                new_action["a"] = a
                new_action["b"] = b
                expanded.append((score - 0.08 * (abs(a) + abs(b) + theta / 10.0), new_action))
        return expanded
    ...
    def _evaluate_candidates(self, balls, table, remaining_targets, candidates):
        ...
        for _, action in candidates:
            ...
            for _ in range(self._rollouts):
                ...
                if shot is None:
                    ...
                    reward = self._analyze_shot_for_reward(shot, last_state_snapshot, remaining_targets)
                total += reward
                n_ok += 1
                if reward <= -140:
                    foul_hits += 1
            if n_ok == 0:
                ...
                avg = total / float(n_ok)
                foul_rate = foul_hits / float(n_ok)
            score = avg - 140.0 * foul_rate
            if score > best_score:
                best_score = score
                best_action = action
